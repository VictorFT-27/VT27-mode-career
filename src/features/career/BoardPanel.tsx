...
export function BoardPanel({ career, onExit }: { career: Career; onExit: () => void }) {
  ...
  return <div className={`board-center board-${board.status}`}>
    ...
    <section className="panel"><div className="section-heading"><div><p className="eyebrow">REUNIÕES PÓS-RODADA</p><h2>Histórico de avaliações</h2></div><span className="muted">{board.history.length} de {leagueRounds.length} rodadas avaliadas</span></div>{!board.history.length ? <p className="muted">A primeira avaliação será registrada ao terminar sua primeira partida do Brasileirão.</p> : <div className="board-history">{[...board.history].reverse().map(review => <article key={review.round}><span className={review.delta >= 0 ? 'review-positive' : 'review-negative'}>{review.delta >= 0 ? '+' : ''}{review.delta}</span><div><strong>Rodada {review.round} · {review.result === 'win' ? 'Vitória' : review.result === 'draw' ? 'Empate' : 'Derrota'} · {review.rank}º lugar</strong><p>{review.reason}</p></div><b>{review.confidence}</b></article>)}</div>}</section>
