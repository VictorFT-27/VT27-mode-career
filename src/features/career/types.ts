...
export type DealType = 'purchase' | 'loan' | 'sale'
export type NegotiationStatus = 'draft' | 'club-review' | 'countered' | 'club-accepted' | 'personal-terms' | 'completed' | 'rejected' | 'withdrawn'
export type NegotiationTerms = { fee: number; installments: number; performanceBonus: number; sellOnPercent: number; loanSeasons?: number; wageShare?: number; purchaseOption?: number; wage?: number; contractSeasons?: number; signingBonus?: number; squadRole?: 'prospect' | 'rotation' | 'starter' | 'key' }
export type Negotiation = { id: string; playerId: string; kind: DealType; fromClubId: string; toClubId: string; status: NegotiationStatus; round: number; terms: NegotiationTerms; counterTerms?: NegotiationTerms; note: string; season: number }
export type TransferDesk = { negotiations: Negotiation[]; listed: { playerId: string; kind: 'sale' | 'loan' }[] }
