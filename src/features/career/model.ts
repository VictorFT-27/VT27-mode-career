import { type Match, type Formation, type Preparation, positions, fixtureDays, leagueDays, leagueRounds, leagueEndDay, leagueResultCount, type LeagueResult, type SeasonArchive, type Contract, type Finances, type TransferRecord, type BoardState, type PlayerAvailability } from './types'
...
export function createCareer(name: string, clubId: string): Career {
  ...
  return { mode: 'coach', name: name.trim(), clubId: safeClubId, formation: '4-3-3', dataVersion: 2, roster, lineup: defaultLineupFor(safeClubId), contracts: defaultContracts(safeClubId), finances: { ...defaultFinances[safeClubId] } }
}
...
export function loadCareer(): Career | null {
  try {
    ...
    const upgradedCompetition = c.dataVersion !== 2 && c.leagueActive === true
