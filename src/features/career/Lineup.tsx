...
export function Lineup({ career, onChange }: { career: Career; onChange: (career: Career) => void }) {
  ...
  return <section className="lineup-workspace">
    ...
    <div className="lineup-layout"><div><fieldset className="formation-pills" disabled={locked}><legend>Formação</legend>{(Object.keys(positions) as Formation[]).map(value => <label className={value === career.formation ? 'chosen' : ''} key={value}><input type="radio" name="formation" checked={career.formation === value} onChange={() => onChange({ ...career, formation: value })} />{value}</label>)}</fieldset><div className="interactive-pitch"><div className="pitch-half" />{rows.map((row, i) => <div className="lineup-row" key={i}>{row.map(slot => { const p = player(ids[slot]); return <button aria-pressed={selected === slot} aria-label={`${slots[slot]}: ${p.name}. ${fit(p.id, slots[slot]) < 1 ? 'Fora da posição natural.' : 'Posição natural.'} Selecionar para trocar.`} className={`pitch-athlete ${selected === slot ? 'selected-athlete' : ''}`} key={slot} onClick={() => setSelected(slot)}><span className="jersey">{overall(career, p.id)}</span><strong>{p.name.split(' ').at(-1)}</strong><small>{energy(career, p.id)}% energia</small><small>{slots[slot]} {fit(p.id, slots[slot]) < 1 && <span title="Fora da posição natural">· !</span>}</small></button> })}</div>)}</div><p className="pitch-help">! Fora da posição natural · Pode reduzir a força do time.</p></div>
    <section className="bench-panel"><p className="eyebrow">POSIÇÃO SELECIONADA · {slots[selected]}</p><h2>{current.name}</h2><p className="muted">{current.position} de origem · nível {overall(career, current.id)}</p><p className="swap-help">Selecione um reserva para entrar. Escolher outro titular troca os dois de posição.</p><div className="candidate-list">{rosterOf(career).filter(p => selected === 0 ? p.position === 'GOL' : p.position !== 'GOL').map(p => <button disabled={locked || ids[selected] === p.id} key={p.id} className={ids.includes(p.id) ? 'candidate starter' : 'candidate'} onClick={() => onChange({ ...career, lineup: swap(ids, selected, p.id) })}><span className="candidate-position">{p.position}</span><span><strong>{p.name}</strong><small>{ids[selected] === p.id ? 'Nesta posição' : ids.includes(p.id) ? 'Titular · trocar posição' : 'Reserva · colocar em campo'}</small></span><b>{overall(career, p.id)}<small>{energy(career, p.id)}%</small></b></button>)}</div><p className="muted">Goleiros só ocupam o gol. Os demais atletas podem ser improvisados.</p></section></div>
