...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="brand" onClick={() => setView('modes')} aria-label="VT27 — início">VT<span>27</span><small>MODE CAREER</small></button><div className="top-meta"><span className="live-dot" /> {view === 'player-office' ? 'CARREIRA DE JOGADOR' : view === 'director-office' ? 'CARREIRA DE DIRIGENTE' : career?.leagueActive ? 'TEMPORADA NACIONAL' : 'PRÉ-TEMPORADA'} <span className="edition">EDIÇÃO 16</span></div></header>
    ...
    {view === 'office' && career && <div className="office"><aside className="sidebar"><div className="club-identity"><Crest club={club} /><h2>{club.name}</h2><p>TREINADOR · {career.name}</p></div><nav aria-label="Área do treinador">{([['overview', '◫', 'Visão geral'], ['squad', '☷', 'Elenco'], ['tactics', '⌘', 'Prancheta'], ['match', '▷', 'Partida'], ['schedule', '▦', 'Calendário'], ['league', '◇', 'Brasileirão'], ['cup', '⬡', 'Copa do Brasil'], ['statistics', '▥', 'Estatísticas'], ['world', '◎', 'Mundo'], ['market', '$', 'Mercado'], ['medical', '+', 'Departamento'], ['board', '◆', 'Diretoria'], ['review', '↗', 'Temporada']] as const).map(([id, icon, label]) => <button disabled={isDismissed(career) && id !== 'board' && id !== 'statistics' && id !== 'world'} key={id} className={tab === id ? 'nav-active' : ''} onClick={() => setTab(id)}><span aria-hidden="true">{icon}</span>{label}</button>)}</nav><div className="sidebar-foot"><span className={`badge ${isDismissed(career) ? '' : 'active'}`}>{isDismissed(career) ? 'DEMITIDO' : 'TREINADOR · ' + boardOf(career).confidence}</span><p>Os três modos de carreira<br />já estão disponíveis.</p><button className="text-button" onClick={() => setView('modes')}>← Seleção de carreira</button></div></aside><main className="dashboard"><div className="dashboard-title"><div><p className="eyebrow">CENTRO DE COMANDO / {club.name}</p><h1 tabIndex={-1} ref={heading}>{tab === 'overview' ? `Bem-vindo, ${career.name}.` : tab === 'squad' ? 'Conheça seu elenco.' : tab === 'tactics' ? 'Sua ideia de jogo.' : tab === 'schedule' ? 'Planeje. Prepare. Evolua.' : tab === 'league' ? 'Agora vale pontos.' : tab === 'cup' ? 'O caminho até a taça.' : tab === 'statistics' ? 'Sua história em números.' : tab === 'world' ? 'O universo segue vivo.' : tab === 'market' ? 'Planeje o próximo reforço.' : tab === 'medical' ? 'Quem está pronto para jogar?' : tab === 'board' ? 'Seu trabalho está em avaliação.' : tab === 'review' ? 'Sua carreira em evolução.' : 'Dia de jogo.'}</h1></div><span className="badge">{career.leagueActive ? 'TEMPORADA NACIONAL' : 'PRÉ-TEMPORADA'} · T{career.seasonNumber ?? 1} · DIA {String(career.day ?? 1).padStart(2, '0')}</span></div>
      ...
      {tab === 'world' && <WorldPanel career={career} onChange={update} />}
