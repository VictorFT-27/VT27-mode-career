...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="brand" onClick={() => setView('modes')} aria-label="VT27 — início">VT<span>27</span><small>MODE CAREER</small></button><div className="top-meta"><span className="live-dot" /> {career?.leagueActive ? 'LIGA VT27' : 'PRÉ-TEMPORADA'} <span className="edition">EDIÇÃO 10</span></div></header>
    ...
    {view === 'setup' && <main className="setup"><button className="text-button" onClick={() => setView('modes')}>← Voltar aos modos</button><p className="eyebrow">CARREIRA DE TREINADOR / 01</p><h1 tabIndex={-1} ref={heading}>O banco de reservas<br /><em>espera por você.</em></h1><p className="muted">Defina seu perfil e escolha onde dar o primeiro passo.</p><form onSubmit={event => { event.preventDefault(); if (!name.trim()) return; update(createCareer(name, clubId)); setTab('overview'); setView('office') }}>
      ...
      <fieldset><legend>Escolha seu primeiro clube</legend><p className="muted">Primeiro grupo de clubes brasileiros com elencos reais da temporada 2026.</p><div className="club-grid">{clubs.map(item => <label key={item.id} className={`club-option ${clubId === item.id ? 'selected' : ''}`}><div className="club-top"><Crest club={item} /><input type="radio" name="club" value={item.id} checked={clubId === item.id} onChange={() => setClubId(item.id)} /></div><h2>{item.name}</h2><span className="muted">{item.city}</span><p className="club-reputation">{item.reputation}</p><p>{item.description}</p><small>OBJETIVO DA DIRETORIA</small><strong>{item.objective}</strong></label>)}</div></fieldset>
