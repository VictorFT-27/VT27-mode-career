...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="brand" onClick={() => setView('modes')} aria-label="VT27 — início">VT<span>27</span><small>MODE CAREER</small></button><div className="top-meta"><span className="live-dot" /> {view === 'player-office' ? 'CARREIRA DE JOGADOR' : view === 'director-office' ? 'CARREIRA DE DIRIGENTE' : career?.leagueActive ? 'TEMPORADA NACIONAL' : 'PRÉ-TEMPORADA'} {career && <button className="help-button" onClick={() => { setView('office'); setTab('settings') }}>SAVE</button>}<button className="help-button" onClick={() => setTutorial(true)}>COMO JOGAR</button><span className="edition">EDIÇÃO 26</span></div></header>
