...
import { clubs, loadCareer, modes, rosterOf, saveCareer, type Career, type Club } from '../features/career/model'

type View = 'modes' | 'setup' | 'office'
type Tab = 'overview' | 'squad' | 'tactics' | 'match' | 'schedule' | 'league' | 'market' | 'review'
...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="brand" onClick={() => setView('modes')} aria-label="VT27 — início">VT<span>27</span><small>MODE CAREER</small></button><div className="top-meta"><span className="live-dot" /> {career?.leagueActive ? 'LIGA VT27' : 'PRÉ-TEMPORADA'} <span className="edition">EDIÇÃO 07</span></div></header>
    ...
    {view === 'setup' && <main className="setup"><button className="text-button" onClick={() => setView('modes')}>← Voltar aos modos</button><p className="eyebrow">CARREIRA DE TREINADOR / 01</p><h1 tabIndex={-1} ref={heading}>O banco de reservas<br /><em>espera por você.</em></h1><p className="muted">Defina seu perfil e escolha onde dar o primeiro passo.</p><form onSubmit={event => { event.preventDefault(); if (!name.trim()) return; update({ mode: 'coach', name: name.trim(), clubId, formation: '4-3-3' }); setTab('overview'); setView('office') }}>
      ...
      <fieldset><legend>Escolha seu primeiro clube</legend><p className="muted">Clubes e elencos fictícios para explorar esta primeira versão.</p><div className="club-grid">{clubs.map(item => <label key={item.id} className={`club-option ${clubId === item.id ? 'selected' : ''}`}><div className="club-top"><Crest club={item} /><input type="radio" name="club" value={item.id} checked={clubId === item.id} onChange={() => setClubId(item.id)} /></div><h2>{item.name}</h2><span className="muted">{item.city}</span><p className="club-reputation">{item.reputation}</p><p>{item.description}</p><small>OBJETIVO DA DIRETORIA</small><strong>{item.objective}</strong></label>)}</div></fieldset>
    ...
    {view === 'office' && career && <div className="office"><aside className="sidebar"><div className="club-identity"><Crest club={club} /><h2>{club.name}</h2><p>TREINADOR · {career.name}</p></div><nav aria-label="Área do treinador">{([['overview', '◫', 'Visão geral'], ['squad', '☷', 'Elenco'], ['tactics', '⌘', 'Prancheta'], ['match', '▷', 'Partida'], ['schedule', '▦', 'Calendário'], ['league', '◇', 'Liga VT27'], ['market', '$', 'Mercado'], ['review', '↗', 'Temporada']] as const).map(([id, icon, label]) => <button key={id} className={tab === id ? 'nav-active' : ''} onClick={() => setTab(id)}><span aria-hidden="true">{icon}</span>{label}</button>)}</nav><div className="sidebar-foot"><span className="badge active">TREINADOR</span><p>Jogador e dirigente<br />chegam nas próximas etapas.</p><button className="text-button" onClick={() => setView('modes')}>← Seleção de carreira</button></div></aside><main className="dashboard"><div className="dashboard-title"><div><p className="eyebrow">CENTRO DE COMANDO / {club.name}</p><h1 tabIndex={-1} ref={heading}>{tab === 'overview' ? `Bem-vindo, ${career.name}.` : tab === 'squad' ? 'Conheça seu elenco.' : tab === 'tactics' ? 'Sua ideia de jogo.' : tab === 'schedule' ? 'Planeje. Prepare. Evolua.' : tab === 'league' ? 'Agora vale pontos.' : tab === 'market' ? 'Planeje o próximo reforço.' : tab === 'review' ? 'Sua carreira em evolução.' : 'Dia de jogo.'}</h1></div><span className="badge">{career.leagueActive ? 'LIGA VT27' : 'PRÉ-TEMPORADA'} · T{career.seasonNumber ?? 1} · DIA {String(career.day ?? 1).padStart(2, '0')}</span></div>
      ...
      {tab === 'squad' && <section className="panel"><div className="section-heading"><h2>Elenco atual</h2><span className="muted">{rosterOf(career).length} atletas · contratos ativos</span></div><div className="table-wrap"><table><thead><tr><th scope="col">Jogador</th><th scope="col">Posição</th><th scope="col">Idade</th><th scope="col">Nível</th><th scope="col">Situação</th><th scope="col">Energia</th><th scope="col">Contrato</th></tr></thead><tbody>{rosterOf(career).map((player, i) => <tr key={player.name}><td><span className="shirt-number">{i + 1}</span>{player.name}</td><td><span className="position">{player.position}</span></td><td>{player.age}</td><td><strong className="rating">{overall(career, player.id)}</strong></td><td>{lineupOf(career).includes(player.id) ? 'Titular' : 'Reserva'}</td><td><span className={energy(career, player.id) < 75 ? 'energy-low' : 'energy-good'}>{energy(career, player.id)}%</span></td><td>{contractsOf(career)[player.id]?.seasons ?? 2} temp.</td></tr>)}</tbody></table></div><button className="secondary" onClick={() => setTab('market')}>Abrir mercado →</button></section>}
      ...
      <p className="save-notice" role="status">{notice || 'Carreira local · salva neste navegador'}</p><footer className="prototype-note">VERSÃO INICIAL · Clubes e atletas fictícios. Pré-temporada e Liga VT27 com simulação simplificada.</footer>
    </main></div>}
  </div>
}
