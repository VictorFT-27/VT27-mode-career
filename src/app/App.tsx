...
import { WorldPanel } from '../features/career/WorldPanel'
import { ageOf } from '../features/career/world'
...
import { PlayerCareer } from '../features/player/PlayerCareer'
import { PlayerSetup } from '../features/player/PlayerSetup'
import { createPlayerCareer, loadPlayerCareer, savePlayerCareer, type PlayerCareer as PlayerCareerState, type PlayerPosition } from '../features/player/playerModel'
import { DirectorCareer } from '../features/director/DirectorCareer'
import { DirectorSetup } from '../features/director/DirectorSetup'
...
export default function App() {
  ...
  const [playerCareer, setPlayerCareer] = useState<PlayerCareerState | null>(loadPlayerCareer)
  const [directorCareer, setDirectorCareer] = useState<DirectorCareerState | null>(loadDirectorCareer)
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="brand" onClick={() => setView('modes')} aria-label="VT27 — início">VT<span>27</span><small>MODE CAREER</small></button><div className="top-meta"><span className="live-dot" /> {view === 'player-office' ? 'CARREIRA DE JOGADOR' : view === 'director-office' ? 'CARREIRA DE DIRIGENTE' : career?.leagueActive ? 'TEMPORADA NACIONAL' : 'PRÉ-TEMPORADA'} {career && <button className="help-button" onClick={() => { setView('office'); setTab('settings') }}>SAVE</button>}<button className="help-button" onClick={() => setTutorial(true)}>COMO JOGAR</button><span className="edition">EDIÇÃO 24</span></div></header>
    ...
    {view === 'modes' && <main className="selection">
      ...
      {playerCareer && <button className="resume player-resume" onClick={() => setView('player-office')}><span>CONTINUAR COMO JOGADOR <strong>{playerCareer.name} · {clubs.find(c => c.id === playerCareer.clubId)?.name}</strong></span><span aria-hidden="true">↗</span></button>}
      {directorCareer && <button className="resume director-resume" onClick={() => setView('director-office')}><span>CONTINUAR COMO DIRIGENTE <strong>{directorCareer.name} · {clubs.find(c => c.id === directorCareer.clubId)?.name}</strong></span><span aria-hidden="true">↗</span></button>}
      <div className="mode-grid">{modes.map(mode => <article key={mode.id} className={`mode-card ${mode.available ? 'available' : ''}`}><div className="card-top"><span className="mode-number">{mode.number}</span><span className={mode.available ? 'badge active' : 'badge'}>{mode.available ? 'DISPONÍVEL · VERSÃO INICIAL' : 'EM DESENVOLVIMENTO'}</span></div><div className={`mode-art ${mode.id}`} aria-hidden="true">{mode.id === 'coach' ? <Pitch /> : <span className="giant-mark">{mode.id === 'player' ? '10' : 'FC'}</span>}</div><p className="eyebrow">{mode.subtitle}</p><h2>{mode.title}</h2><p>{mode.description}</p><button className={mode.available ? 'primary' : 'locked'} disabled={!mode.available} onClick={() => { setNotice(''); setName(''); if (mode.id === 'coach') setView('setup'); if (mode.id === 'player') setView('player-setup'); if (mode.id === 'director') setView('director-setup') }}>{mode.available ? `Começar como ${mode.title.toLowerCase()}` : 'Disponível em breve'}{mode.available && <span aria-hidden="true">↗</span>}</button></article>)}</div>
      <div className="selection-footer"><span>UM UNIVERSO. TRÊS PERSPECTIVAS.</span><span>Primeira etapa: sua chegada ao clube.</span></div>
    </main>}
    {view === 'player-setup' && <PlayerSetup name={name} clubId={clubId} position={playerPosition} number={shirtNumber} replacing={!!playerCareer} onName={setName} onClub={setClubId} onPosition={setPlayerPosition} onNumber={setShirtNumber} onBack={() => setView('modes')} onStart={() => { const next = createPlayerCareer(name, clubId, playerPosition, shirtNumber); updatePlayer(next); setView('player-office') }} />}
    {view === 'player-office' && playerCareer && <PlayerCareer career={playerCareer} onChange={updatePlayer} onExit={() => setView('modes')} />}
    {view === 'director-setup' && <DirectorSetup name={name} clubId={clubId} replacing={!!directorCareer} onName={setName} onClub={setClubId} onBack={() => setView('modes')} onStart={() => { const next = createDirectorCareer(name, clubId); updateDirector(next); setView('director-office') }} />}
    {view === 'director-office' && directorCareer && <DirectorCareer career={directorCareer} onChange={updateDirector} onExit={() => setView('modes')} />}
